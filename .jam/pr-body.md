...
- Fixes post-picker PR handoff so the base branch is resolved from the Picker worktree, preserving `master` when it exists while using `main` for Jamboree's main-only repo.
...
- `cargo test -p jam-svc-worktree -p jam-task-lifecycle` - passed, 23 tests.
