...
Live probe note (2026-05-18): `jam-svc-observe` now augments `query-quota` with non-spending live harness probes for subscription harnesses. Codex uses `codex login status`; Claude Code uses `claude auth status --json`. These probes establish account reachability/tier freshness in real time and synthesize unknown-headroom rows when no journal/config quota row exists. Remaining subscription counters still come from observed quota events/config because the first-party CLIs do not expose a stable no-spend remaining-quota endpoint.
