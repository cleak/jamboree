...
Author-attribution note (2026-05-11): observed that orchestrator-opened PR `cleak/blueberry#391` came back with `author.is_bot:true, login: app/jamboree-cleak`, which causes CodeRabbit to silently skip its auto-review (no `.coderabbit.yaml` opt-in exists; the comment-trigger workaround is one-shot and gets stuck behind CodeRabbit's incremental-skip). Per the addendum on `dec-github-app-not-pat`, `jam-svc-repo` now also accepts a user-to-server token at `jam/pickers/github-user-token` (or env `JAM_GITHUB_USER_TOKEN`) and prefers it on the write path (`gh pr create`, `git push`, PR-comment writes). The token is seeded via `scripts/authorize-github-user-token.sh` (GitHub device flow against the App's Client ID). Acceptance criterion for closing this task now also covers: orchestrator-opened PRs land with `author.is_bot:false` matching the authorizing user, and CodeRabbit auto-reviews them through the normal path on initial open and on each push.
