...
JAMBOREE_REPO="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
...
# ---------------------------------------------------------------------------
# Skills directory in the Jamboree monorepo (per dec-skills-in-monorepo-v1)
# ---------------------------------------------------------------------------

prepare_skills_dir() {
    # Per dec-skills-in-monorepo-v1: skills live at $JAMBOREE_REPO/skills/.
    # Group-shared with maestro (mode 2770) so the orchestrator can write
    # record-learning outputs and skill evolution candidates while caleb
    # remains the owner.
    if [[ ! -d "$SKILLS_DIR" ]]; then
        warn "Skills directory not found at $SKILLS_DIR
       (Expected to exist as part of the monorepo. Continuing — will create empty.)"
        if [[ $DRY_RUN -eq 0 ]]; then
            run_cmd install -d -o "$HUMAN_USER" -g "$MAESTRO_USER" -m 2770 "$SKILLS_DIR"
        fi
    fi

    # Ownership: human user owns; maestro group has access.
    run_cmd chown -R "$HUMAN_USER:$MAESTRO_USER" "$SKILLS_DIR" 2>/dev/null || true

    # Mode 2770 with setgid on directories so new files inherit group=maestro.
    if [[ $DRY_RUN -eq 0 ]]; then
        find "$SKILLS_DIR" -type d -exec chmod 2770 {} \; 2>/dev/null || true
        find "$SKILLS_DIR" -type f -exec chmod 660 {} \; 2>/dev/null || true
    else
        run_cmd find "$SKILLS_DIR" -type d -exec chmod 2770 {} \;
        run_cmd find "$SKILLS_DIR" -type f -exec chmod 660 {} \;
    fi

    pass "Skills tree at $SKILLS_DIR set to caleb:maestro mode 2770 (setgid)"
}

write_skills_config() {
    # Per dec-skills-direct-read-with-config: the Maestro reads skills directly
    # from configured paths. Default config points at the monorepo's skills/
    # tree plus Blueberry's CLAUDE.md / AGENTS.md as individual file entries.
    local config_dir
    config_dir="$(dirname "$SKILLS_CONFIG")"

    if [[ -f "$SKILLS_CONFIG" ]]; then
        pass "Skills config already exists: $SKILLS_CONFIG"
        return
    fi

    if [[ $DRY_RUN -eq 1 ]]; then
        info "[dry-run] would write $SKILLS_CONFIG"
        return
    fi
    ...
    tmpfile="$(mktemp)"
    cat > "$tmpfile" <<EOF
# /home/$MAESTRO_USER/.jam/config/skills.toml
#
# Generated by bootstrap-users.sh (per dec-skills-direct-read-with-config).
# The Maestro reads skills directly from the paths below — no symlink, no sync.
# Edits to any file or new files in any folder are picked up on the next
# read-skills(scope) call (and hot-invalidated via inotify).

[skills]
# Folders walked recursively for *.md skill files.
# Each file's scope is determined by its YAML front-matter (\`scope:\` field),
# falling back to its path if absent.
folders = [
    "$SKILLS_DIR",
]

# Individual files to load. Useful for pulling project-side documentation
# (Blueberry's CLAUDE.md / AGENTS.md) in as scoped skills without duplication.
files = [
    "/home/$HUMAN_USER/blueberry/CLAUDE.md",
    "/home/$HUMAN_USER/blueberry/AGENTS.md",
]
EOF

    install -d -o "$MAESTRO_USER" -g "$MAESTRO_USER" -m 750 "$config_dir"
    install -m 640 -o "$MAESTRO_USER" -g "$MAESTRO_USER" "$tmpfile" "$SKILLS_CONFIG"
    rm -f "$tmpfile"
    pass "Skills config written: $SKILLS_CONFIG"
}
...
verify_users() {
    ...
    # Skills tree perms (per dec-skills-in-monorepo-v1)
    if [[ -d "$SKILLS_DIR" ]]; then
        local skills_mode skills_owner skills_group
        skills_mode="$(stat -c '%a' "$SKILLS_DIR")"
        skills_owner="$(stat -c '%U' "$SKILLS_DIR")"
        skills_group="$(stat -c '%G' "$SKILLS_DIR")"
        if [[ "$skills_mode" == "2770" && "$skills_owner" == "$HUMAN_USER" && "$skills_group" == "$MAESTRO_USER" ]]; then
            pass "$SKILLS_DIR mode 2770 $skills_owner:$skills_group (correct)"
        else
            fail "$SKILLS_DIR perms wrong: $skills_owner:$skills_group mode $skills_mode (want $HUMAN_USER:$MAESTRO_USER mode 2770)"
            failed=1
        fi
    else
        warn "$SKILLS_DIR not found — skills tree missing"
    fi

    # Skills config exists
    if [[ -f "$SKILLS_CONFIG" ]]; then
        pass "Skills config exists: $SKILLS_CONFIG"
    else
        fail "Skills config missing: $SKILLS_CONFIG"
        failed=1
    fi

    # Maestro can read the skills tree (defense-in-depth)
    if [[ -d "$SKILLS_DIR" ]] && [[ $DRY_RUN -eq 0 ]]; then
        if sudo -n -u "$MAESTRO_USER" ls "$SKILLS_DIR" >/dev/null 2>&1; then
            pass "$MAESTRO_USER can read $SKILLS_DIR"
        else
            warn "$MAESTRO_USER cannot read $SKILLS_DIR — group membership may need refresh (logout/login)"
        fi
    fi
...
main() {
    ...
    header "Preparing skills tree (per dec-skills-in-monorepo-v1)"
    prepare_skills_dir
    write_skills_config
...
Next steps:
  ...
  5. Add other orchestrator secrets to maestro's pass store
     (GitHub PAT, ntfy creds, etc. — see security-setup.md §5.3 / spec §11.3.1):
        ...
        # ... repeat for each key
  6. Create the canonical Tempyr worktree for Blueberry (per dec-blueberry-jam-path):
        sudo -u $HUMAN_USER -i git -C /home/$HUMAN_USER/blueberry \
            worktree add $BLUEBERRY_JAM_DIR tempyr-live
        sudo chown -R $HUMAN_USER:$MAESTRO_USER $BLUEBERRY_JAM_DIR
        sudo find $BLUEBERRY_JAM_DIR -type d -exec chmod 2770 {} \;
        sudo find $BLUEBERRY_JAM_DIR -type f -exec chmod 660 {} \;
        # Note: Blueberry uses graph/ (not tempyr/) per its .tempyr/config.toml.
        # The orchestrator writes graph/tasks/ on lifecycle transitions.
  7. Run: jam setup    (which now also verifies this user-isolation layout)
